...
age_class_indx <- c(intvl_step_yr,#fawns
                    intvl_step_yr*2,#1
                    intvl_step_yr*3,#2
                    intvl_step_yr*4,#3
                    intvl_step_yr*6,#4-5
...
for(i in 1:nrow(d_fit_hh)){
    if(d_fit_hh$right_age_smonth[i] > intvl_step_yr*9){
        d_fit_hh$ageclassmort[i] <- 7
    }else{
        d_fit_hh$ageclassmort[i] <- min(which(d_fit_hh$right_age_smonth[i]<=age_class_indx))
    }
}
...
#hunting season for collared data
...
for (i in 1:n_year_collar) {
    startng[i] <- interval(startdate,
        min(d_huntseason$OpenDate[d_huntseason$Year == (i + 2016)])) %/% months(1)+1
    endng[i] <- interval(startdate,
        max(d_huntseason$CloseDate[d_huntseason$Year == (i + 2016)])) %/% months(1)+1
    startgun[i] <- interval(startdate,
        d_huntseason$OpenDate[d_huntseason$Year == (i + 2016) &
                                d_huntseason$SeasonType == "nineday"]) %/% months(1)+1
...
for (i in 1:length(unique(d_season$Year))) {
    season_ng_start[i] <- ceiling(as.duration(ymd("1994-05-15") %--% ymd(min(d_season$OpenDate[d_season$Year == (i + 1993)])))/dmonths(1))
    season_ng_end[i] <- ceiling(as.duration(ymd("1994-05-15") %--% ymd(max(d_season$CloseDate[d_season$Year == (i + 1993)])))/dmonths(1))
}

# pulling out approximate 9 day gun season
temp <- d_season %>% filter(SeasonType == "nineday")
nonineday <- c(1994:2020)[which(!(c(1994:2020) %in% temp$Year))]
temp2 <- d_season %>% 
...
for (i in 1:length(unique(temp$Year))) {
    season_gun_start[i] <- ceiling(as.duration(ymd("1994-05-15") %--% ymd(temp$OpenDate[temp$Year == (i + 1993)]))/dmonths(1))
    season_gun_end[i] <- ceiling(as.duration(ymd("1994-05-15") %--% ymd(temp$CloseDate[temp$Year == (i + 1993)]))/dmonths(1))
}
...
for(t in 1:n_year){
   d_fit_season[t,] <- c(intvl_step_yr*(t-1)+1,season_ng_start[t]-1,season_ng_start[t],season_gun_start[t],season_gun_end[t],season_ng_end[t],season_ng_end[t]+1,intvl_step_yr*t) 
}
...
Z_cause_ng <- rep(0,nT_period_collar_m)
Z_cause_gun <- rep(0,nT_period_collar_m)
for(i in 1:5){
    Z_cause_ng[startng[i]:endng[i]] <- 1
    Z_cause_gun[startgun[i]:endgun[i]] <- 1
}
...
Z_overall_ng <- rep(0,nT_period_overall_m)
Z_overall_gun <- rep(0,nT_period_overall_m)
for(i in 1:n_year){
    Z_overall_ng[d_fit_season$ng_start[i]:d_fit_season$ng_end[i]] <- 1
    Z_overall_gun[d_fit_season$gun_start[i]:d_fit_season$gun_end[i]] <- 1
}
